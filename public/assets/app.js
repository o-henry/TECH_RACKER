(() => {
  ...
  const messages = {
    ...
    all: '전체', medical: '의료·생명과학', ai: 'AI·소프트웨어', energy: '에너지·전력', climate: '기후·환경', quantum: '양자·컴퓨팅', robotics: '로봇·모빌리티', semiconductors: '반도체·광통신', manufacturing: '첨단소재·제조', space: '우주·통신', food: '식품·농업', other: '기타',
  ...
  const GROUP_LABEL_KEYS = {
    '전체': 'all', '의료·생명과학': 'medical', 'AI·소프트웨어': 'ai',
    '에너지·전력': 'energy', '기후·환경': 'climate', '양자·컴퓨팅': 'quantum',
    '로봇·모빌리티': 'robotics', '반도체·광통신': 'semiconductors',
    '첨단소재·제조': 'manufacturing', '우주·통신': 'space', '식품·농업': 'food', '기타': 'other'
  };
  ...
  const GROUPS = ['전체', 'AI·소프트웨어', '의료·생명과학', '반도체·광통신', '양자·컴퓨팅', '로봇·모빌리티', '에너지·전력', '기후·환경', '첨단소재·제조', '우주·통신', '식품·농업'];
  ...
  function groupOf(tech) {
    ...
    if (/^(에너지|전력)/.test(category)) return '에너지·전력';
    if (/^(기후|환경)/.test(category)) return '기후·환경';
    if (/^(양자|컴퓨팅·양자)/.test(category)) return '양자·컴퓨팅';
    if (/^(로봇|모빌리티)/.test(category)) return '로봇·모빌리티';
    if (/^(반도체|광통신)/.test(category)) return '반도체·광통신';
    if (/^(첨단소재|제조)/.test(category)) return '첨단소재·제조';
    if (/^(우주|통신)/.test(category)) return '우주·통신';
    if (/^(식품|농업)/.test(category)) return '식품·농업';
    return '기타';
  }
  ...
  function renderDeepAnalysisResult(item) {
    ...
    return `
      ...
      <article class="deep-analysis-outlook">
        <header><span class="mono">TECHNOLOGY OUTLOOK</span><h3>기술의 향후 전개 경로</h3></header>
        <p class="outlook-scope">${escapeHTML(result.technologyOutlook?.scope || '검증된 기술 전망이 없습니다.')}</p>
        <div class="outlook-pathways">
          ${(result.technologyOutlook?.pathways || []).map((entry, index) => `
            <section>
              <div class="outlook-path-head"><span class="mono">P${String(index + 1).padStart(2, '0')}</span><strong>${escapeHTML(entry.label)}</strong><time class="mono">${escapeHTML(entry.horizon)}</time></div>
              <p>${escapeHTML(entry.mechanism)}</p>
              <dl>
                <div><dt>필요 조건</dt><dd><ul>${(entry.requiredConditions || []).map(value => `<li>${escapeHTML(value)}</li>`).join('')}</ul></dd></div>
                <div><dt>관측 신호</dt><dd><ul>${(entry.observableSignals || []).map(value => `<li>${escapeHTML(value)}</li>`).join('')}</ul></dd></div>
                <div><dt>경로 무효화 신호</dt><dd><ul>${(entry.invalidationSignals || []).map(value => `<li>${escapeHTML(value)}</li>`).join('')}</ul></dd></div>
              </dl>
            </section>`).join('')}
        </div>
        <small class="outlook-note mono">BASIS ${escapeHTML(result.technologyOutlook?.basisDate || item.verifiedThrough || '미확인')} · NO INVESTMENT VIEW · NO FABRICATED PROBABILITY</small>
      </article>
