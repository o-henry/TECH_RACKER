...
def _candidate_source_class(document: SourceDocument) -> str:
    metadata = document.metadata_json if isinstance(document.metadata_json, dict) else {}
    if document.source_type == "rss" and metadata.get("signal_class") == "public_interest":
        return "interest_signal"
    return SOURCE_CLASS.get(document.source_type, "other")
...
async def enqueue_new_candidates(session: AsyncSession, *, limit: int = 1_000) -> int:
    ...
    for document, relevance in rows:
        source_class = _candidate_source_class(document)
        if source_class in {"reviewed_feed", "interest_signal"}:
            technology_ids = list(
                (
            ...
            if technology_ids:
                recent_related = await session.scalar(
                    ...
                    .where(
                        AnalysisCandidate.source_class == source_class,
                        AnalysisCandidate.created_at >= utcnow() - timedelta(
                            hours=24 if source_class == "interest_signal" else 6
                        ),
        ...
        session.add(
            AnalysisCandidate(
                ...
                priority=min(
                    1.0,
                    (0.35 if source_class == "interest_signal" else SOURCE_PRIORITY.get(document.source_type, 0.5))
                    + float(relevance or 0) * 0.05,
                ),
