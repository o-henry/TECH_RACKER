...
export function validateDeepAnalysisResult(value: unknown):
  ...
  const relationshipAnalysis = Array.isArray(input.relationshipAnalysis)
    ...
    : [];
  const outlookInput = input.technologyOutlook as Record<string, unknown> | undefined;
  const outlookPathways = Array.isArray(outlookInput?.pathways)
    ? outlookInput.pathways.slice(0, 5).map((item) => {
        const row = item as Record<string, unknown>;
        return {
          label: cleanString(row.label, 180),
          horizon: cleanString(row.horizon, 120),
          mechanism: cleanString(row.mechanism, 2_000),
          requiredConditions: cleanStringList(row.requiredConditions, 8, 1_000),
          observableSignals: cleanStringList(row.observableSignals, 8, 1_000),
          invalidationSignals: cleanStringList(row.invalidationSignals, 8, 1_000),
        };
      }).filter((item) =>
        item.label && item.horizon && item.mechanism &&
        item.requiredConditions.length && item.observableSignals.length && item.invalidationSignals.length
      )
    : [];
  const technologyOutlook = {
    basisDate: cleanString(outlookInput?.basisDate, 40),
    scope: cleanString(outlookInput?.scope, 2_000),
    pathways: outlookPathways,
  };
  ...
  if (
    ...
    technologyOutlook.scope.length < 60 ||
    technologyOutlook.pathways.length < 2 ||
